...
void output15(unsigned char* str);
inline void output2(unsigned char* str);
/****************************** MACROS ******************************/
...
inline bool addmore(struct note* head0, BYTE* output, int index, int mode, unsigned char* record) {
	...
	if (k == index) {
		printf("The string below is part of the collision:\n");
		output15( (**((struct endnote**)pointer1)).str );
		output2((**((struct endnote**)pointer1)).str);
	}
	return (k == index);
...
inline void test2(int index,int index2) {
	unsigned char* str;
	str = (unsigned char*)malloc(15 * sizeof(char)); convert(0, str);
	struct note* head0;
	...
	for (lli = 0; lli < 123456789012345; ++lli) {
		...
		sha1_final(&ctx, buf);
		if (lli < index2) {
			if (addmore(head0, buf, index, 1, str)) {
			...
			if (addmore(head0, buf, index, 2, str)) {
	...
	unsigned char* str1;
	str1 = (unsigned char*)malloc(115 * sizeof(char));
	convert(lli, str1);
	printf("The string below is part of the collision:\n");
	output15(str1);
	output2(str1);
...
int main()
{
	...
	test2(2, 100000);
	printf("\nTest the results:\n");
	output2((convert(166698, str), output15(str), str));
	output2((convert(1178713, str), output15(str), str));
	output2((convert(104864, str), output15(str), str));
	output2((convert(41737, str), output15(str), str));
	output2((convert(1084, str), output15(str), str));
	output2((convert(4878, str), output15(str), str));
	output2((convert(53, str), output15(str), str));
	output2((convert(527, str), output15(str), str));
