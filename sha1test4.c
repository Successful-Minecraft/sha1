	...
	for(i=0;i<index;++i){
		if (mode == 1) {
			...
			if (j != 0) {
				...
				if(i!=index-1){
				struct note* newnote = new struct note;
				construct(newnote);
				(**pointer1).next[output[i]] = newnote;
				pointer1 = &(**pointer1).next[output[i]];
				}
				else if (i == index - 1) {
					struct endnote* newnote = new struct endnote;
...
int main()
{
	unsigned char* str = new unsigned char[15];
	...
	convert(0, str);
	struct note *head0= new struct note;
	construct(head0);
