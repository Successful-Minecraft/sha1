...
inline bool addmore(struct note** head0, BYTE* output, int firstNnibbles) {
	int i = 0, j = 0, k = 0;
	struct note* pointer = (struct note*)malloc(sizeof(struct note));
	if (head0[output[0]] == NULL) {
		struct note* newnote = (struct note*)malloc(sizeof(struct note));
		head0[output[0]] = newnote;
		
	}
	pointer = head0[output[0]];
	for (i = 1; i < 20; ++i){
		if (pointer == NULL)
			pointer = construct();
		
		struct note* newnote2 = (struct note*)malloc(sizeof(struct note));
		pointer->next[output[i]] = newnote2;
		pointer = pointer->next[output[i]];
	}
